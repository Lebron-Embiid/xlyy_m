<!DOCTYPE html>
<html lang="en">
<head>
	<meta charset="UTF-8">
	<title>已关注</title>
	<meta name="keywords" content="关键词">
	<meta name="description" content="网站描述">
	<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, user-scalable=no" />
	<meta name="renderer" content="webkit">
	<link rel="stylesheet" href="css/bootstrap.min.css">
	<link rel="stylesheet" href="css/normalize.css">
	<link rel="stylesheet" href="css/common.css">
	<link rel="stylesheet" href="css/person.css">
	<script src="js/rem.js"></script>
	<script src="js/jquery-2.1.4.min.js"></script>
	<script src="js/bootstrap.min.js"></script>
</head>
<body>
	<div class="header_back">
		<a href="javascript:history.go(-1);"><img src="images/back.png" alt=""></a>
		<h2>已关注</h2>
	</div>
	<div class="mt80"></div>
	<div class="atten_box">
		<div class="atten_till">
			<div><span></span>已关注会员10/10</div>
			<p>关注人数已到上限，升级关注更多</p>
		</div>
		<ul class="atten_list">
			<li>
				<div class="left">
					<img src="images/atten_img1.jpg" alt="">
					<div class="word">
						<h4>小情</h4>
						<h6>28、江苏省盐城市</h6>
						<p>期待奢华的生活方式</p>
					</div>
				</div>
				<div class="right">
					<a href=""><img src="images/each.png" alt="">互相关注</a>
				</div>
			</li>
			<li>
				<div class="left">
					<img src="images/atten_img2.jpg" alt="">
					<div class="word">
						<h4>小情</h4>
						<h6>28、江苏省盐城市</h6>
						<p>期待奢华的生活方式</p>
					</div>
				</div>
				<div class="right">
					<a href="" class="def_atten">+关注</a>
				</div>
			</li>
			<li>
				<div class="left">
					<img src="images/atten_img3.jpg" alt="">
					<div class="word">
						<h4>小情</h4>
						<h6>28、江苏省盐城市</h6>
						<p>期待奢华的生活方式</p>
					</div>
				</div>
				<div class="right">
					<a href="" class="def_atten">已关注</a>
				</div>
			</li>
			<li>
				<div class="left">
					<img src="images/atten_img4.jpg" alt="">
					<div class="word">
						<h4>小情</h4>
						<h6>28、江苏省盐城市</h6>
						<p>期待奢华的生活方式</p>
					</div>
				</div>
				<div class="right">
					<a href=""><img src="images/each.png" alt="">互相关注</a>
				</div>
			</li>
			<li>
				<div class="left">
					<img src="images/atten_img5.jpg" alt="">
					<div class="word">
						<h4>小情</h4>
						<h6>28、江苏省盐城市</h6>
						<p>期待奢华的生活方式</p>
					</div>
				</div>
				<div class="right">
					<a href="" class="def_atten">+关注</a>
				</div>
			</li>
			<li>
				<div class="left">
					<img src="images/atten_img6.jpg" alt="">
					<div class="word">
						<h4>小情</h4>
						<h6>28、江苏省盐城市</h6>
						<p>期待奢华的生活方式</p>
					</div>
				</div>
				<div class="right">
					<a href="" class="def_atten">已关注</a>
				</div>
			</li>
		</ul>
	</div>
	<?php include "footer.php"; ?>
</body>
</html>